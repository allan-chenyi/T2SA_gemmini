#!/bin/bash
# 64x64 conv workload runner.
#
# Runs spad-only workloads on Baseline64x64WS and Twist64x64WSSingleOp
# simulators, 16 Verilator threads each.
#
# Prerequisites:
#   64x64 simulators built (already available in sims/verilator/).
#
# Usage:
#   bash run_conv_64x64.sh             # full flow
#   bash run_conv_64x64.sh --extract   # skip sims, just extract
set -eo pipefail
...
LOGDIR="$EVALDIR/logs_64x64"

DIM_VAL=64
B_SIM="${SIMDIR}/simulator-chipyard.harness-Baseline64x64WSRocketConfig"
T_SIM="${SIMDIR}/simulator-chipyard.harness-Twist64x64WSSingleOpRocketConfig"
...
for logf in sorted(logdir.glob("*.log")):
    fname = logf.stem  # e.g. baseline_resnet_bottleneck_spad
...
for test in tests:
    ...
    if br and tr:
        ...
        print("  %-24s %-10s %11.2f%% %11.2f%% %11.2f%%" % (
...
# -----------------------------------------------------------------------
if [[ "$1" == "--extract" ]]; then
    echo "=== Extracting conv 64x64 results ==="
    extract_results "$LOGDIR" "$EVALDIR/data/conv_64x64.csv"
    exit 0
fi
...
for SIM in "$B_SIM" "$T_SIM"; do
    if [[ ! -f "$SIM" ]]; then
        ...
        echo "Build 64x64 simulators first."
        exit 1
    fi
done
...
# -----------------------------------------------------------------------
# 2. Build RISC-V binaries with DIM=64 params
# -----------------------------------------------------------------------
...
echo "============================================"
echo "  Logs: $LOGDIR/"
echo "  CSV:  $EVALDIR/data/conv_64x64.csv"
