...
int main (int argc, char * argv[]) {
    ...
    // Unified E2E report. Conv has no explicit preload phase, so cold==warm
    // == the tiled_conv_auto window; general covers the full main including
    // CPU-side setup, allocation, and im2col.
    uint64_t bench_general_end = read_cycles();
    uint64_t bench_compute = end_gemmini - start_gemmini;
