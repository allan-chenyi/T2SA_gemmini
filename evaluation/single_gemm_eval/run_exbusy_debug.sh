#!/bin/bash
# Single-GEMM EX_BUSY sweep: run simulations, extract data, plot.
#
# Prerequisites:
#   - Simulators built: bash build_rtl.sh
#   - Test binary compiled (this script builds it automatically)
#
# What it does:
#   1. Build single_gemm_exbusy test binary (RISC-V bare-metal)
#   2. Run baseline + twist simulations in parallel
#   3. Extract EXBUSY_SWEEP + MESH_PIPELINE data from logs → CSV
#   4. Plot speedup figure
#
# Output:
#   logs/exbusy_debug/{baseline,twist}.log
#   data/{exbusy,mesh_pipeline}.csv
...
echo "============================================"
echo "  Data:    $EVALDIR/data/mesh_pipeline.csv"
