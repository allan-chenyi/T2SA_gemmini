#!/usr/bin/env python3
"""
Plot Single-GEMM Compute Sweep results (combined figure, vertical layout).
...
def plot_combined(data, D):
    ...
    ax1.set_ylabel("Cycle count", fontsize=FS_LABEL)
    ax1.set_xticks(x)
    ...
    for Q in Q_vals:
        ...
        for M in M_vals:
            bk = ("baseline", M, Q)
            tk = ("twist", M, Q)
            if bk in data and tk in data and data[bk] > 0:
                md_val = M / D
                if md_val >= MD_MIN:
                    md_m.append(md_val)
                    sp_m.append(100.0 * (data[bk] - data[tk]) / data[bk])
        if md_m:
    ...
    for ext in ("pdf", "png"):
        fig.savefig(FIGDIR / ("gemmini_sweep_combined.%s" % ext),
                    dpi=300 if ext == "pdf" else 150, bbox_inches="tight")
    print("  Saved gemmini_sweep_combined.pdf/png")
    plt.close(fig)
...
def print_summary(data, D):
    M_vals = sorted({m for (c, m, q) in data if c == "baseline"})
    Q_vals = sorted({q for (c, m, q) in data if c == "baseline"})
    print("\n  Cycle saving (baseline - twist), averaged:")
    header = "  %6s" % "M\\Q"
    for Q in Q_vals:
        header += " %6d" % Q
    print(header)
